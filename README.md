...
| プロンプトファイル | 用途 | タイミング |
|-------------------|------|------------|
| `issue.txt` | **Issue本文**でのメンション | Issue作成時や説明更新時 |
| `issue_comment.txt` | **Issueコメント**でのメンション | Issue内での質問や追加要求 |
| `pr.txt` | **PR本文**でのメンション | PR作成時や説明更新時 |
| `pr_comment.txt` | **PRコメント**でのメンション | レビューコメントや修正要求 |

#### 🎯 **各プロンプトの特徴**

- **`issue.txt`**: 包括的な実装（新機能・バグ修正）
- **`issue_comment.txt`**: 部分的な修正・clarification
- **`pr.txt`**: コードレビューと品質改善
- **`pr_comment.txt`**: レビューフィードバックへの対応

#### ⚙️ **テンプレート変数**

すべてのプロンプトで `{{USER_REQUEST}}` が実際のメンション内容に置換されます。
